...
export default async function MyListingsPage() {
  ...
  return (
    <div className="mx-auto max-w-um-content px-4 pb-24 pt-8 sm:px-6 sm:pt-10 lg:px-8 lg:pb-28 lg:pt-14">
      <header className="grid gap-8 border-b border-black/10 pb-10 lg:grid-cols-[1fr_auto] lg:items-end">
        <div>
          ...
          <h1 className="mt-4 text-[clamp(2.7rem,7vw,5.5rem)] font-bold leading-[0.92] tracking-[-0.06em] text-um-text-strong">
            Your listings.
            <span className="font-editorial ml-2 font-normal italic">All in one place.</span>
          </h1>
          <p className="mt-5 max-w-2xl text-sm leading-7 text-um-text-muted sm:text-base">
            Continue a draft, keep live listings accurate, or clear out anything that has found a
            new home.
          </p>
        </div>

        <Link
          className="inline-flex min-h-12 w-fit shrink-0 items-center justify-center gap-2 rounded-um-sm bg-um-gold-500 px-5 text-sm font-bold text-um-ink-950 shadow-um-xs transition duration-160 ease-um-out hover:-translate-y-0.5 hover:bg-um-gold-400 hover:shadow-um-sm"
          href="/listings/new"
        >
      ...
      {listings.length === 0 ? (
        ...
        <div className="space-y-16 pt-12 sm:pt-14">
          {GROUPS.map((group) => {
...
function ManagedListingCard({ listing }: { listing: ManagedListing }) {
  ...
  return (
    <article className="group overflow-hidden rounded-um-lg border border-black/10 bg-white shadow-um-xs transition duration-220 ease-um-out hover:-translate-y-0.5 hover:shadow-um-sm">
...
function EmptyListings() {
  return (
    <section className="relative mx-auto max-w-2xl overflow-hidden py-20 text-center sm:py-28">
      <span className="mx-auto grid size-14 place-items-center rounded-um-md bg-um-ink-950 text-um-gold-400 shadow-um-sm">
        <Store aria-hidden="true" className="size-6" strokeWidth={1.8} />
      </span>
      <p className="font-condensed mt-6 text-xs font-bold uppercase tracking-[0.16em] text-um-gold-700">
        Your first listing
      </p>
      <h2 className="font-editorial mt-2 text-3xl tracking-[-0.035em] text-um-text-strong sm:text-4xl">
        Pass something on.
      </h2>
      <p className="mx-auto mt-4 max-w-lg leading-7 text-um-text-muted">
        Start a private draft, add a few clear photos, and publish whenever it feels ready.
      </p>
      <Link
        className="mt-7 inline-flex min-h-12 items-center gap-2 rounded-um-sm bg-um-gold-500 px-5 text-sm font-bold text-um-ink-950 shadow-um-xs transition duration-160 ease-um-out hover:-translate-y-0.5 hover:bg-um-gold-400 hover:shadow-um-sm"
        href="/listings/new"
      >
